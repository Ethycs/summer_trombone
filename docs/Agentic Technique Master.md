# Agentic Technique Master

**Status:** CURRENT
**Date:** 2026-08-20
**Applies to:** Documentation, tests, and reference implementations in this repository
**Depends on:** None

Artificial Intelligence can be a tool that frees us from reinventing the wheel, or it can be a yoke that keeps us in mundane tasks forever.
...
Run `rg -L '^\*\*Status:\*\*' docs -g '*.md'` from the repository root to find Markdown documents without a status block, then classify each result in the appropriate evidence lane before treating it as actionable documentation.
